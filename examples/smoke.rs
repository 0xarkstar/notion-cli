...
use notion_cli::api::block::AppendBlockChildrenRequest;
...
use notion_cli::types::block::BlockBody;
...
use notion_cli::validation::{BlockId, DatabaseId};
use serde_json::json;
...
async fn run() -> Result<(), String> {
    ...
    // --- 4b. Block append + list + delete (v0.2 block CRUD) --------------
    let page_as_block = BlockId::parse(new_page_id.as_str())
        .map_err(|e| format!("page->block id: {e}"))?;
    let append_req = AppendBlockChildrenRequest::new(vec![
        BlockBody::heading_1("Smoke Test Content"),
        BlockBody::paragraph("Generated by notion-cli examples/smoke.rs"),
        BlockBody::bulleted("First item"),
        BlockBody::bulleted("Second item"),
        BlockBody::to_do("Review results", false),
        BlockBody::code("let answer = 42;", "rust"),
        BlockBody::divider(),
    ]);
    let appended = step(
        "append_block_children",
        client.append_block_children(&page_as_block, &append_req),
    )
    .await?;
    println!("   → appended {} block(s)\n", appended.results.len());

    let listed = step(
        "list_block_children",
        client.list_block_children(&page_as_block, None, Some(20)),
    )
    .await?;
    if listed.results.len() < 7 {
        return Err(format!(
            "expected at least 7 blocks, got {}",
            listed.results.len()
        ));
    }
    println!("   → listed {} block(s)\n", listed.results.len());

    // Delete the first appended block to verify delete endpoint
    if let Some(first) = listed.results.first().and_then(|b| b.as_known()) {
        let block_id = first.id.clone();
        step("delete_block", client.delete_block(&block_id)).await?;
        println!("   → deleted one block\n");
    }
